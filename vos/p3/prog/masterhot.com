$!****************************************************************************
$!
$! Build proc for MIPL module masterhot
$! VPACK Version 1.8, Thursday, October 28, 2004, 00:42:45
$!
$! Execute by entering:		$ @masterhot
$!
...
$!****************************************************************************
$!
$ write sys$output "*** module masterhot ***"
$!
$ Create_Source = ""
$ Create_Repack =""
$ Create_PDF = ""
...
$ write sys$output "Invalid argument given to masterhot.com file -- ", primary
...
$!
$Run_Make_File:
$   if F$SEARCH("masterhot.imake") .nes. ""
$   then
$      vimake masterhot
$      purge masterhot.bld
$   else
$      if F$SEARCH("masterhot.bld") .eqs. ""
$      then
$         gosub Imake_File
$         vimake masterhot
$      else
...
$      @masterhot.bld "STD"
$   else
$      @masterhot.bld "''primary'" "''secondary'"
$   endif
$ Return
$!#############################################################################
$Repack_File:
$ create masterhot.repack
...
$ DECK/DOLLARS="$ VOKAGLEVE"
C
C	VICAR program MASTERHOT, used to calculate upwelling radiance at
C	surface from radiance at sensor for the MASTER instrument, thermal IR
C	channels (Bands 41-50).
C
C	This program is identical to MASTERTIR, except that it outputs 
C	fullword pixels, rather than halfword pixels.
C
...
C	Revision: 1.4   Change to fullword output             rea 10/26/04
C
	INCLUDE 'VICMAIN_FOR'
	SUBROUTINE MAIN44
C
...
C
	REAL TRANX(716,10),PATHX(716,10),BUF(716)
	INTEGER OUTBUF(716)
C
	DO ICHAN=1,10
		...
		DO J=1,716
		    OUTBUF(J) = NINT((1000.0*BUF(J)-PATHX(J,ICHAN)) /
     +				TRANX(J,ICHAN))
		END DO
...
ten MASTER TIR channels (Channels 41 to 50), in units of milliWatts per
sqare meter per steradian per micrometer.  Output pixels are integer
(32 bit signed integer).  The units of the downwelling sky irradiance (values
...
$!#############################################################################
$Imake_File:
$ create masterhot.imake
#define  PROGRAM   masterhot

#define MODULE_LIST masterhot.f
