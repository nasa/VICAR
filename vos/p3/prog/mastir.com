$!****************************************************************************
$!
$! Build proc for MIPL module mastir
$! VPACK Version 1.8, Friday, May 21, 1999, 17:10:50
$!
$! Execute by entering:		$ @mastir
$!
...
$!****************************************************************************
$!
$ write sys$output "*** module mastir ***"
$!
$ Create_Source = ""
$ Create_Repack =""
$ Create_PDF = ""
...
$ write sys$output "Invalid argument given to mastir.com file -- ", primary
...
$!
$Run_Make_File:
$   if F$SEARCH("mastir.imake") .nes. ""
$   then
$      vimake mastir
$      purge mastir.bld
$   else
$      if F$SEARCH("mastir.bld") .eqs. ""
$      then
$         gosub Imake_File
$         vimake mastir
$      else
...
$      @mastir.bld "STD"
$   else
$      @mastir.bld "''primary'" "''secondary'"
$   endif
$ Return
$!#############################################################################
$Repack_File:
$ create mastir.repack
...
$ DECK/DOLLARS="$ VOKAGLEVE"
C
C	VICAR program MASTIR, used to calculate upwelling radiance at
C	surface from radiance at sensor for the MAS instrument, thermal IR
C	channels (Bands 42-50).
C
C	Ron Alley	27 August 1998
C	Revision: 1.1   Report calibration file name
C	Revision: 1.2   Get filenames from modtran input file   rea 5/21/99
C	Revision: 1.2.1 Update MODTRAN3 argument list           rea 7/02/01
C
	INCLUDE 'VICMAIN_FOR'
	SUBROUTINE MAIN44
C
	REAL RESP(900,9),TRANX(716,9),PATHX(716,9)
...
C						Open datasets, get size field
	...
	CALL XVOPEN(IOUT,ISTAT,'IO_ACT','SA','OPEN_ACT','SA',
     ...
     +		   'U_ORG','BSQ','U_NB',9,'U_NL',NL,'U_NS',NS,' ')
...
C							      update VICAR label
	CALL XLADD(IOUT,'HISTORY','LBL1','MAS Ground Radiance Image',
     +		   ISTAT,'FORMAT','STRING',' ')
...
C
	...
	REAL ANGF(50),F(4,50),FRAC(716),TRANS(9,2),PATH(9,2),SKY(9,2)
	INTEGER ICLD1(LAYDIM),IREG(4)
	LOGICAL LDISORT,LSUN1
...
C						  for each sample location
	...
	DO I=1,358
	    ...
	    FRAC(I) = (COSFULL*(1.0-COSI)) / (COSI*(1.0-COSFULL))
	    FRAC(717-I) = FRAC(I)
	END DO
...
C					the 9 normalized response functions.
	    DO ICHAN=1,9
...
C					  put sky radiance values in VICAR label
	CALL XLADD(IOUT,'HISTORY','SKY42',SKY(1,1),ISTAT,
     +		   'FORMAT','REAL',' ')
	CALL XLADD(IOUT,'HISTORY','SKY43',SKY(2,1),ISTAT,
     +		   'FORMAT','REAL',' ')
	CALL XLADD(IOUT,'HISTORY','SKY44',SKY(3,1),ISTAT,
     +		   'FORMAT','REAL',' ')
	CALL XLADD(IOUT,'HISTORY','SKY45',SKY(4,1),ISTAT,
     +		   'FORMAT','REAL',' ')
	CALL XLADD(IOUT,'HISTORY','SKY46',SKY(5,1),ISTAT,
     +		   'FORMAT','REAL',' ')
	CALL XLADD(IOUT,'HISTORY','SKY47',SKY(6,1),ISTAT,
     +		   'FORMAT','REAL',' ')
	CALL XLADD(IOUT,'HISTORY','SKY48',SKY(7,1),ISTAT,
     +		   'FORMAT','REAL',' ')
	CALL XLADD(IOUT,'HISTORY','SKY49',SKY(8,1),ISTAT,
     +		   'FORMAT','REAL',' ')
	CALL XLADD(IOUT,'HISTORY','SKY50',SKY(9,1),ISTAT,
     +		   'FORMAT','REAL',' ')
C					     interpolate transmittance and path
C					     radiance to a value for each sample
	DO ICHAN=1,9
	    DO I=1,358
		TRANX(I,ICHAN) = FRAC(I)*TRANS(ICHAN,1) +
     +				 (1.0-FRAC(I))*TRANS(ICHAN,2)
		PATHX(I,ICHAN) = FRAC(I)*PATH(ICHAN,1) +
     +				 (1.0-FRAC(I))*PATH(ICHAN,2)
		TRANX(717-I,ICHAN) = TRANX(I,ICHAN)
		PATHX(717-I,ICHAN) = PATHX(I,ICHAN)
	    END DO
	END DO
C							      report the relults
	...
	DO ICHAN=1,9
	    WRITE (MSG,500) ICHAN+41,TRANS(ICHAN,1),TRANS(ICHAN,2),
...
C
	REAL TRANX(716,9),PATHX(716,9),BUF(716)
	INTEGER*2 OUTBUF(716)
C
	DO ICHAN=1,9
	    DO ILINE=ISL,ISL+NL-1
		CALL XVREAD(IN,BUF,ISTAT,'LINE',ILINE,'BAND',ICHAN,' ')
		DO J=1,716
		    NUM = NINT((1000.0*BUF(J)-PATHX(J,ICHAN)) /
     +				TRANX(J,ICHAN))
...
.LEVEL1
.VARI INP
(1) MAS TIR radiance at
    sensor image, Bands 42-50
(2) MAS TIR calibration file
.VARI OUT
Output data set of the 9 MAS
...
$Imake_File:
$ create mastir.imake
#define  PROGRAM   mastir

#define MODULE_LIST mastir.f
