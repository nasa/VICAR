...
/*	STATE TABLES							*/
    ...
    struct 	STATE			/* state structure		*/
    	...
    	CODE	(*action)(TEXT*, struct LOCVAL *);		/* pointer to action routine	*/
...
(
    ...
    else if (isextletter((int) string[0]))		/* NAME criteria */
    ...
    for (i=0; string[i]!=EOS; i++)
      if (!isdigit((int) string[i]))		/* not an integer */
	...
	if (string[0]=='e' || string[0]=='E') { 
	  ...
	  if (string[i-1]=='e' || string[i-1]=='E') {
	    ...
	    if (string[0]!='e' && string[0]!='E') class = INTEINT;
	  }
	}
      }
    }
 ...
 */
    ...
    if ((check =  s_s2i1(&token[i+1], &exp)) != SUCCESS) return(check); /* convert into exp */
    if ((*locval).expsign == NEGATIVE) exp = -exp;
    if ((temp = power(10.0, exp)) == 0) return(PA_ERRFLOW);
    code = fl_mult((*locval).realval, temp,&(*locval).realval);
    if (code==FAIL) code = PA_ERRFLOW;
 ...
 */
    ...
    tint[i] = EOS;			/* terminate the string			*/
    check = s_s2r1(tint, &(*locval).realval);	/* convert and put in local value 	*/
    if (check != SUCCESS) return(check);
    if (token[i] == EOS || token[i+1] == EOS) return(check);	/* return if no E		*/
