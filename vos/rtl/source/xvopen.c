...
/* Preset the OPEN_ACT and OPEN_MES parameters in case there's an early error */
   ...
   status = v2_open_act_preset_c(unit, nargs, &params);
   va_end(params);
   if (status != SUCCESS) {
...
void FTN_NAME2(xvopen, XVOPEN) (int *unitp, int *status, ...)		/*$$*/
{
...
/* Also, fill in nopts if we don't know yet...takes place of get_nopts().  */
   ...
   *status = v2_open_act_preset_for(unit, &nopts, &nargs, argno, &params);
   if (*status != SUCCESS) {
      va_end(params);
      v2_error_handler(unit, *status);
      return;
