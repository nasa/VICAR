...
int v2_read_tape(state, buf, block, nblocks, async_flag, transfer_count)
...
int *transfer_count;
{
   ...
   *transfer_count = 0;
   ...
   if (delta != 0) {
      if (space_record(state, delta) != SUCCESS)      /* adjusts i_rec[index] */
   ...
   for (i=0; i<nblocks; i++) {
	 ...
	 *transfer_count += state->iosb.transfer_count;
	 ...
	 if (code == SS$_ENDOFFILE) {
	    code = space_record(state,-1);
	    if (code != SUCCESS)
	       return code;		/* override EOF message */
	    if (*transfer_count == 0)
