...
$! VPACK Version 1.8, Friday, August 09, 1996, 15:53:15
$!
...
$ write sys$output "Invalid argument given to rotate.com file -- ", primary
...
.page
 ...
 REVISION HISTORY
  ...
  96-8-9    SP   Changed to use only LGEOM because MGEOM is iffy.
.level1
.vari inp
...
ZZPAR is still created, so the user may run LGEOM or MGEOM with 
...
before running the ROTATE job the user inputs

DCL DEFINE LGIDS SCX:[USER]LGIDS

then all subsequent ROTATE jobs would point to that Intermediate data
