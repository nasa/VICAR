...
C Input and output images may be byte, halfword, fullword, real*4, or real*8.
...
C If OCODE>=4, the output line is REAL.  Otherwise it is INTEGER*4.
C
      SUBROUTINE INTRPV(OCODE,NSO,ILO,IHI,C,D,BUF1,BUF2,obuf)
      IMPLICIT NONE
      INTEGER*4 OCODE,NSO,ILO,IHI
      REAL*8 C,D
      REAL*8 BUF1(NSO),BUF2(NSO),OBUF(NSO)

      INTEGER*4 I,DN

      IF (OCODE.GE.4) GOTO 50
      DO I=1,NSO
