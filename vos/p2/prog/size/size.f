...
C
      ...
      REAL*8 GSCALE		!Output DN scale factor

c      COMMON/C2/RBUF(100000),BUF(100000,2),OBUF(100000)
c      COMMON/C2/SAMP(100000),WGHT(100000)
      INTEGER*4 SAMP(100000)
      REAL*8 BUF(100000),RBUF(100000),OBUF(100000),WGHT(100000)

      INTEGER*4 I,L,N,IND,INCODE
      LOGICAL*4 INTERP
      CHARACTER*4 FMT(5)/'BYTE','HALF','FULL','REAL','DOUB'/
  
      CALL XVMESSAGE('*** SIZE version 2017-11-21 ***',' ')
...
C     ....If format conversion is required, reopen input
      ...
      IF (INTERP) INCODE=5
c	force all input to be converted to real*8 on the fly
         CALL XVCLOSE(IUNIT,IND,' ')
         CALL XVOPEN(IUNIT,IND,'OPEN_ACT','SA','IO_ACT','SA',
     &		'I_FORMAT',FMT(ICODE),'U_FORMAT',FMT(5),' ')		!FMT(INCODE),' ')
...
C     ....If there is an offset, write top lines before offset
      DO I=1,NSOUT
         OBUF(I) = 0.
      ENDDO
C     ZIA clears 4*n, but OBUF is *8
      IF (ICODE.LT.4) CALL ZIA(OBUF,2*NSOUT)
      DO L=1,SLO-1
...
C     ....If output size is larger than sized input, write remaining lines
	 ...
	 GOTO 100
      ...
      ENDDO
C     ZIA clears 4*n, but OBUF is *8
      IF (OCODE.LT.4) CALL ZIA(OBUF,2*NSOUT)
...
C     ....Determine max and min limits of output DN
      ...
      CALL XVPARMD('SCALE',GSCALE,INUM,IDEF,1)  !Optional scaling of output DNs
      IF (IDEF.NE.1) THEN 
