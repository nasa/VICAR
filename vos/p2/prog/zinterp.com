...
$! VPACK Version 1.7, Monday, November 21, 1994, 11:55:43
$!
...
C	SSORTP		 Sort in an in-core array
...
600		IF (NUMVALID.LT.NMAX) THEN
		...
		ELSE
		    ...
		    END IF
	            ...
	            CALL SSORTP(DBUFF,1,NUMVALID,IP)
                    DO K = 1, NUMVALID
                       DBUFF(K) = DBUFF2(IP(K))
                       DKEY(K) = DKEY2(IP(K))
                    END DO
