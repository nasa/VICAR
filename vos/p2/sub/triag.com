$!****************************************************************************
$!
$! Build proc for MIPL module triag
$! VPACK Version 1.5, Monday, December 14, 1992, 08:58:06
$!
$! Execute by entering:		$ @triag
$!
...
$!
$Run_Make_File:
$   if F$SEARCH("triag.imake") .nes. ""
$   then
$      vimake triag
$      purge triag.bld
$   else
$      if F$SEARCH("triag.bld") .eqs. ""
$      then
$         gosub Imake_File
$         vimake triag
$      else
...
$ vpack triag.com -
	-s triag.f -
	-i triag.imake -
	-t ttriag.f ttriag.imake ttriag.pdf tsttriag.pdf -
	-o triag.hlp
$ Exit
$ VOKAGLEVE
$ Return
...
$Imake_File:
$ create triag.imake
/* Imake file for VICAR subroutine  TRIAG   */

#define SUBROUTINE  triag

#define MODULE_LIST  triag.f  
...
C  PROGRAM TTRIAG
C  
C  This is a test program for subroutine TRIAG.
C  TRIAG packs 2 RESEAU data sets into GEOMA format. 
C  The routine is specific to VOYAGER.
c
c
...
c
      Do  I = 4,7
        ...
        DO L = 1,23
          ...
          Call TRIAG(L,ISLIN,OSLIN,GEOMAB)
          Call Prnt(7,96,GEOMAB,' OUTPUT GEOMA ORDERED BUFFER.')
        EndDO
      EndDO
c
      Return
...
$!-----------------------------------------------------------------------------
$ create ttriag.imake
/* IMAKE file for Test of VICAR subroutine  TRIAG   */

#define PROGRAM  ttriag

#define MODULE_LIST ttriag.f 
...
Let $echo="yes"
TTRIAG
!THIS IS A TEST OF SUBROUTINE TRIAG.
!TRIAG PACKS TWO RESEAU DATA SETS INTO GEOMA FORMAT.
TTRIAG
End-proc
$ Return
$!#############################################################################
...
1 TRIAG
      ...
      To pack two reseau data sets into GEOMA format.  This routine
      is specific to VOYAGER.

2  CALLING SEQUENCE

      CALL TRIAG(LINRES,ISR,OSR,GEOMAB)

2  ARGUMENTS

      LINRES   Row of reseaus to be packed     I*4

      ISR      Image reseau line               R*4

      OSR      Object reseau line              R*4

      GEOMAB   Output GEOMA ordered buffer     R*4
...
2  HISTORY
      ...
      Ported for UNIX Conversion:  Dec-14-1992  by W.P. Lee
$ Return
$!#############################################################################
