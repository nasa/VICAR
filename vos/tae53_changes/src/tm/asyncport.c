...
FUNCTION VOID free_atcb(struct ATCB *atcb);
	...
	for (acb = acb_head; acb != NULL; acb = (*acb).link)
		...
		if ((*acb).active)
		    ...
		    tmmsg (SUCCESS, "Job '%s' aborted.", "TAE-ABORTOK",
			   (uintptr_t) (*acb).name, 0, 0, 0, 0);
		    }
		else if (!all)
		    {
		    found = TRUE;
		    tmmsg (PROCFAIL, "Job '%s' not active.",
			   "TAE-NOABO", (uintptr_t) (*acb).name,
			   0, 0, 0, 0);
		    }
    		}
	    }
    ...
    if (acbcnt == MAXACB)		/* too many			*/
    	...
    	for (ac = acb_head; (*ac).active; ac = (*ac).link)
    		...
    		tmmsg (PROCFAIL, "No room for new job,  MAXACB jobs active.",
		       "TAE-NOASYROOM", 0, 0, 0, 0, 0);
    		return(FAIL);
    		}
    	    }
    	tmmsg(SUCCESS, "No room.  Removing job '%s' from async job list.",
	      "TAE-REMOVE", (uintptr_t) (*ac).name, 0, 0, 0, 0);    	    	
    	(*ac2).link = acb;
    	(*acb).link = (*ac).link;
...
(
    ...
    tcb = (*(*acb).atcb).tcb;
    code  = asy_recs(tcb, (GENPTR) bufadr, buflen, NULL, NULL);
    if (code != SUCCESS)
    	return(FAIL);
#ifdef 	VAX_VMS
	...
	for (acb = acb_head; acb != NULL; acb = (*acb).link)
		...
		if (!(*acb).active)
		    ...
		    tmmsg (PROCFAIL, "Job '%s' not removed because active.",
			   "TAE-NOABO", (uintptr_t) (*acb).name, 0, 0, 0, 0);
		    last_acb = acb;
		    }
    		}
	    else
...
/*	Tutor on the parameters						    */
    ...
    if ((*request_msg).type == MO_GETPAR)
      asy_getp (&tempctx, (struct PARBLK*) &parblk, sizeof ( parblk.pool ) );	/* Do async getpar proc.    */
    else
	{
	asytut_req = TRUE;			/* dynget for async job     */
                                         	/* Do dynamic tutor proc.   */
	dynget (&tempctx, (struct PARBLK*) &parblk, sizeof ( parblk.pool ), asytut_req );
	}
    curproc = saved_cur;
	    ...
	    if (code == KILLED)
	      tmmsg (PROCFAIL, "User interrupt entered.", "TAE-ASYINTRPT",
	        ...
	        tmmsg(PROCFAIL,
		      ...
		      "TAE-ASYSND", (uintptr_t) (*t).hostcode, 0, 0, 0, 0);
    	    return(DO_CHECK);
    	    }
#ifdef VAX_VMS
...
(
    ...
    if (code != SUCCESS)
	{
    	tmmsg (PROCFAIL, "Error creating async job.  Host error code = %d.",
	       "TAE-ASYCRE",(uintptr_t) (*(*atcb).tcb).hostcode, 0, 0, 0, 0);
    	return(FAIL);
    	}
...

    ...
    addstr2 (&(*cmdctx).locst, "_PARENT", 1, 1, 
	    (TEXT**) (*job_gbl).v_cvp, V_LOCAL);
    addstr2 (&(*cmdctx).locst, "_CHILD", 1, 1, targetvv, V_LOCAL);
#ifdef UNIX
    ...
    code = package (cmdctx, (struct PARBLK*) &parblk, sizeof parblk.pool, 0);
    if (code != SUCCESS)
        {
   ...
   if (code != SUCCESS)
	...
	tmmsg (PROCFAIL, "Error creating async process.  Host error code %d.",
	       "TAE-ASYCRE", (*(*atcb).tcb).hostcode, 0, 0, 0, 0);
	return (FAIL);
	}
...
/* update with possible new parm qualifiers */
	...
	if (tv1 == NULL)
	    ...
	    tmmsg (PROCFAIL, "Error restoring '%s.%s'.", "TAE-ARESTERR",
		   (uintptr_t) (*var).v_name, (uintptr_t) (*vp).v_name,
		   0, 0, 0);
	    return (FAIL);
	    }
...
/* Scan the list to see if any jobs are non-existent; if so return	    */
	...
	for (vnum = 0; vnum < (*job).v_count; vnum++)	 /* search JOB  list */
		...
		{
		tmmsg (PROCFAIL, "Job '%s' not in job list.", "TAE-NOJOB",
		       (uintptr_t) SVAL(*job, vnum), 0, 0, 0, 0);
		return(DO_CHECK);
		}
    ...
    for (vnum = 0; vnum < (*job).v_count; vnum++)        /* search JOB  list */
        ...
        for (acb = acb_head; acb != NULL; acb = (*acb).link)
    		...
    		if ((*acb).waiting)
    		    {
    		    tmmsg (SUCCESS,
"Cannot wait on '%s'; job is waiting for REPLY.", "TAE-REPWAIT", 
			   (uintptr_t) (*acb).name, 0, 0, 0, 0);
    		    if ( (s_equal(SVAL(*job, 0), "ALL")) )
    		        continue;
    		    else
