...
/*  IMPORTANT NOTE REGARDING SIGNALS
 *
 *  The Cartographic group when running TAE as part of AFIDS in batch mode
 *  on fast machines discovered that TAE would frequently but randomly crash.
 *  After much investigation (primarily by Walt Bunch), the culprit was
 *  identified as in the signal handling.  Fixing this seems to have solved
 *  the problem, at least mostly.  We believe the issue is a race condition
 *  that simply never came up on older, slower machines.
 *
 *  Description from Walt:
 *
 *  Once the parent turns the child loose by writing to its pipe (around line
 *  377, inside the code1 == 0) branch:
 *
 *     count = write(write_chan, &p[already_wrote], count );
 *
 *  the parent is vulnerable to being interrupted.  Anywhere.  So before
 *  letting the child loose, SIG_BLOCK is signaled to block signals from the
 *  child.  The only time it is unblocked is when entering the wait loop,
 *  toward the end of the function.  If the parent is signaled in the wait
 *  loop, it longjumps toward the earlier part of the function, where it
 *  immediately signals a SIG_BLOCK again.
 *
 *  rgd 2/2010
 */

