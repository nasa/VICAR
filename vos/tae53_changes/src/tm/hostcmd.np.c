...
FUNCTION VOID term_sig(int UNUSED(X));
FUNCTION CODE shell_do
(
...
FUNCTION CODE fd_do(struct CONTXT* UNUSED(X1), struct CONTXT* UNUSED(X2));
...
#define N 100
	...
	if (fd[i] >= 0)
	    close (fd[i]);    
    tmmsg (SUCCESS, "%d", "TAE-FDSAVAIL", (uintptr_t) n,0,0,0,0);
