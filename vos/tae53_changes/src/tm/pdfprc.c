...
(
    ...
    if (getvrb(&sb, verb) == S_SYNERR) 	/* get verb from cmd str		*/
	{
        tmmsg(PROCFAIL, "Incorrect command format in _ONFAIL command: %s.",
    	      "TAE-ONFAIL", (uintptr_t)sb.errmsg,0,0,0,0);
	...
	if (((*itrcmd).flags & Y_ONFAIL)  != 0)
	    return (SUCCESS);
    tmmsg(PROCFAIL, "'%s' not allowed as _ONFAIL command.", "TAE-ONFAIL", (uintptr_t)verb,0,0,0,0);
    return (FAIL);
    }

/*
...
/* Save the parblk in the communication file				*/
    ...
    code = save_pfile (&sf, (*parblk).msgtyp, NULL, 0, &(*parblk).symtab, 0, 0);	/* write parmst to file */
...
gp_rderr:
    if (code == F_EOF)
      tmmsg(PROCFAIL, "Premature end of proc definition file.", "TAE-PREEOF",0,0,0,0,0);
    else
	tmmsg(PROCFAIL, "Error reading proc definition file. %s",
	      "TAE-RDERR", (uintptr_t)(*pdffil).errmsg,0,0,0,0);
    sttmt[0] = EOS;
    return(FAIL);
    }

 ...
 */
FUNCTION struct IPBLOCK *int_find
(
    struct CONTXT	*ctx,		/* in: proc context 		    */
...
(
	...
	if (gv == NULL)				/* define new global	*/
		...
		"Global variable name '%s' conflicts with level 0 variable.",
		       "TAE-LOCCONFLICT", (uintptr_t)(*pv).v_name,0,0,0,0);
...
(
	...
	tmmsg (PROCFAIL, 
		"Variable '%s' not compatible with variable '%s'.",
	       "TAE-MISMATCH", (uintptr_t)dname, (uintptr_t)sname,0,0,0);
        return (code);
	}
...
outval_error:
    tmmsg(PROCFAIL, "Invalid output value for '%s' in proc '%s'.",
	  "TAE-INVOUT", (uintptr_t)(*v).v_name, (uintptr_t)(*procctx).pdf.name,
	  0,0,0);
    return(FAIL);
    }

...
pd_synerr:
    tmmsg(PROCFAIL, sb.errmsg, "TAE-FMTERR",0,0,0,0,0);
    return(FAIL);

pd_niter:
    tmmsg(PROCFAIL, "Unrecognized command '%s'.",
	  "TAE-UNRECCMD", (uintptr_t)cmd,0,0,0,0);
    return(FAIL);
    }

/*
...
/* before hierarchy search, see if the proc is internal		*/
	...
	if (code != SUCCESS)
	    ...
	    tmmsg (PROCFAIL, "Error finding internal proc. Code '%s'.",
		   "TAE-INTPDF", (uintptr_t)(*file).errmsg,0,0,0,0);
	    return (FAIL);
	    }
	...
	tmmsg (PROCFAIL, "Error in proc name at or near '%s'.",
	       "TAE-PROCSYN", (uintptr_t)errstr,0,0,0,0);
	return (FAIL);
	}
    else if (code == F_FILERR)
	{
	tmmsg(PROCFAIL, "Proc '%s' in '%s':  %s.",
	      "TAE-PDFRD", (uintptr_t)(*pctx).pdf.name, 
	      (uintptr_t)(*pctx).pdf.libr,
	      (uintptr_t)(*file).errmsg,0,0);
	return (FAIL);
	}
    else
	{
	tmmsg(PROCFAIL, "Unable to locate proc '%s'.", "TAE-NOPROC",
	      (uintptr_t)cmdstring,0,0,0,0);
	return (FAIL);
	}
    }
...
(
	...
	if (NULLSTR((*pctx).subcmd)  &&  (*pctx).special != SUB_SEARCH)	/* subcommand present?	*/
		...
		{
		  tmmsg(PROCFAIL, "Subcommand required.", "TAE-SUBREQ",0,0,0,0,0);
		code = FAIL;	
		}
	    }
	else
...
(
    ...
    if (!s_equal(buf, inibuf))
      tmmsg(PROCFAIL, buf, "TAE-SUBAVL",0,0,0,0,0);
    return;
    }

/*
...
(
    ...
    CODE		code;
    CODE		code1, code2;
    static	int processRunning = FALSE;
	...
	if (processRunning)
	    {
	    tmmsg (PROCFAIL, 
		   "Attempt to nest process execution.", "TAE-NOSYNC",0,0,0,0,0); 
	    return (DO_CHECK);	
	    }
	processRunning = TRUE;
...
reset:
    ...
    if ((*pctx).proctype == Y_PROCESS && code ==FAIL)	/* if process was run, saved errmsg */
      tmmsg(PROCFAIL, (*errmsg).text, (*errmsg).key, (uintptr_t)(*errmsg).code,0,0,0,0);
