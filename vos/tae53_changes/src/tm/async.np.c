 ...
 *	18-apr-94	Concurrent port: declare sigblock...dgf
 *
 ****************************************************************************
 */
...
/* MAXENV increased to support the environment HUNGRY VICAR software, I ask */
/* Is it really necessary to set 120 environment variables? */
/* #define MAXENV 100 */
#define MAXENV 1000
 ...
 */
FUNCTION static CODE act_process 
(
...
(
    ...
    set_component (v, (GENPTR) svv, 1);
				/* index of 1 will give 1st component */
    return;
    }

/*
 ...
 *
 */
FUNCTION static VOID build_name 
(
...
/* Must be bigger than PNAMESIZ because it's stract'ed to and THEN truncated. */
#else
...
(
    ...
    COUNT               lun = 0;            /* not used in VMS or UNIX      */
    ...
    if (code != SUCCESS)
        ...
        tmmsg (PROCFAIL, "Unable to create job file '%s'. %s",
               "TAE-JOBOPN", (uintptr_t) file_spec, 
	       (uintptr_t) (*job).errmsg, 0, 0, 0);
        return (FAIL);
        }
    return (SUCCESS);
    }
 ...
 int		sig	/* Signal number */
    ...
    if (count != sizeof (struct MON_MSG))               /* none there, error */
        ...
        tmmsg (PROCFAIL,
	       ...
	       "TAE-ASYRCV", (uintptr_t) errno, 0, 0, 0, 0);
				/* internal error    */
        return;                                         
        }
