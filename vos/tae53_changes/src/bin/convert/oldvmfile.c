 ...
 */
	...
	TEXT		padding[300];	/* so we can read new par files */
	} header;
    CODE		code;
    COUNT		recsize;
    struct  VM_STRUCT   *vm;
...

    ...
    */
    ...
    if (code != SUCCESS)
        goto bad_open;
    code = f_bread(&f, (GENPTR)&header, sizeof(header), &recsize);  
    (*np).hostcode = (code == SUCCESS) ? code : f.host_code;	/* save error code  */
    if (code != SUCCESS)
        goto bad_read;
    if (s_equal (header.ph.sentinel, "<<new par-file>>"))
       goto new_format;
    if (!s_equal(header.ph.sentinel, P_SENTINEL))
       goto bad_format;
...
new_format:
	...
	"Parameter file '%s' has already been converted.", 
		"TAE-VNEWPFILE", (uintptr_t)filespec, 0, 0);
    return (P_FAIL);
    }

 ...
 */
	...
	code = f_bread(f, (GENPTR) &p, sizeof(p), &recsize);
   	if (code == F_EOF) 
	    ...
	    goto bad_read;
	old_makeabs(&p.symtab, p.pool);		/* make pointers absolute */
	if (old_chk_parblk((struct PARBLK *)&p) != SUCCESS)
...
static FUNCTION GENPTR Vm_ValueMove (
...
FUNCTION struct VARIABLE * Vm_VarMove(
	...
	if (Vm_CopyValid((*p).v_type, (*p).v_valid, (*rp).v_valid, (GENPTR)s)!=SUCCESS)
	    return (NULL);
	}
    if (class == V_GLOBAL)
	{
