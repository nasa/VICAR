...
/* TPL CHECKOUT FILE_TIME=15-JUL-1987 19:16 DUA1:[TAEV2.OLB.GENLIB]VMCOPY.C;1 */
/*
 *	Routines for copying VARIABLE structures.
 *      (An outboard version of tae$tm:vcopy.c)  
 *	CHANGE LOG:
...
FUNCTION GENPTR Vm_AllocValue(FAST struct VARIABLE *v);
FUNCTION CODE Vm_CopyValue(FUNINT, GENPTR, GENPTR, FUNINT);
FUNCTION CODE Vm_MoveSymtab(struct SYMTAB *, struct SYMTAB *);
FUNCTION GENPTR Vm_AllocValid(FAST struct VARIABLE *, FUNINT);
FUNCTION CODE Vm_CopyValid(FUNINT,GENPTR,GENPTR,GENPTR);
FUNCTION COUNT Vm_ValidSize(FAST FUNINT, FAST FUNINT);

FUNCTION VOID Vm_FreeTable(FAST struct SYMTAB *);
FUNCTION struct VARIABLE *Vm_AllocVar(struct SYMTAB *);
 ...
 */
	...
	;				/* find end of chain ...*/
        (*v3).v_link = v2;			/* link in new struc	*/
	}
    return(SUCCESS);
    }
