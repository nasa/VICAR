...
#include	"compiled.inc"		/* for proc compilation		*/
    ...
    GLOBAL	v51savr = 0;		/* version number		 */
 ...
 */
    ...
    FUNCTION  CODE  restore_do(cpctx, npctx)
    ...
    TEXT		*targets[MAXVAL];
    TEXT		*vars[MAXVAL];	
    struct  VARIABLE    *lookex(), *srchrf();
    BOOL		e_occur();
    BOOL		select;		
    BOOL		display, all;
    BOOL		parm_display, local_display,
			global_display, all_display, do_display;
    COUNT		i, varcnt;
...
/*  read the parameter records and restore locals/globals/parms		*/
    ...
    for (i=0; i < varcnt; i++)
	if (vars[i] != NULL)
	    tmmsg (PROCFAIL, "Variable '%s' not found.", "TAE-NOVAR", vars[i]);
    return (DO_CHECK);

format_err:
    f_close(&f, F_KEEP);
...

	...
	if (all_classes || s_equal(subc, "PARMS"))
	    st3 = &(*cpctx).parmst;
	code = save_pfile(&f, 0, NULL, 0, 3, st1, st2, st3);
 ...
 *
 */
    FUNCTION CODE save_sel (procctx, list, f)
    ...
    if (qual_save)
	code = save_pfile(f, 0, NULL, 0, 1, &(*v).v_qualst);
    else
        code = save_pfile(f, 0, vector, (*list).v_count, 0);	
    if (code != SUCCESS)
    	{
	tmmsg(PROCFAIL, "Unable to write to save file, %s",
		"TAE-SAVEWRT", (*f).errmsg);
 ...
 *
 */
    FUNCTION CODE set_options (cmdctx, var, varcnt, target, select,
    				global, local, parm)
	...
	*parm = s_equal (subc, "PARMS");
	...
	if (p3 != NULL && (*p3).v_count != 0)
	    {
	    tmmsg(PROCFAIL,
	    "TARGET parameter not allowed with VARIABLE = 'ALL'.", "TAE-BADCOMBO");
	    return(FAIL);
	    }
    	}
    else
	...
	*select = *global = *local = *parm = TRUE;
	...
	if (p3 != NULL && (*p3).v_count != 0)	/* target present?	*/
    		...
    		{
    		tmmsg(PROCFAIL,
    		"VARIABLE count must equal TARGET count.", "TAE-BADTARG");
    		return(FAIL);
    		}
 ...
 */
    ...
    FUNCTION static CODE set_quals (targ, q_st)
...
format_err:
	...
	"TAE-MISMATCH", (*sqv).v_name, (*targ).v_name);
    return (FAIL);
 ...
 */
    ...
    FUNCTION  CODE  symb_up (ctx, var, targ)
    ...
    struct  VARIABLE	*lookex(), *alpha_var(), *usearch(), *allvar();

    type = (*var).v_type;
    class = (*var).v_class;
    if (targ == NULL)		
...

format_err:
    tmmsg(PROCFAIL, "Mismatch restoring '%s'.", "TAE-MISMATCH", (*var).v_name);
    return (FAIL);
...
/* RGD mod 4-29-94 */
/* This is truly incredible coding practice!  The save_pfile() function	*/
/* actually accepts a varying number of arguments, but does not use	*/
/* either <varargs.h> or <stdarg.h>!!!!  I am really surprised that	*/
/* this works at all on RISC processors; they're lucky there are 6	*/
/* arguments or it would fail on a Sparc!  Since it does seem to work	*/
/* in most places, and the portability of this syntax is unclear, this	*/
/* patch is made only for the Alpha (where it failed previously).	*/
/* Late note (7/12/00): It also now fails on SGI with IRIX 6.x so we	*/
/* enable it there too.  It was also enabled for HP some time ago.	*/
#if (defined(__VMS) && defined(__ALPHA)) || defined(hpux) || defined(sgi)
#define FIX_PFILE_BUG
#endif

#ifdef FIX_PFILE_BUG
#include <varargs.h>

    FUNCTION CODE save_pfile (sf, msgtype, vector, vcnt, scnt, va_alist)
    ...
    FUNINT		scnt;		/* in: number of sym tabs following  */
    va_dcl
#else
    FUNCTION CODE save_pfile (sf, msgtype, vector, vcnt, scnt, symtab)
    ...
    FUNINT		vcnt;		/* in: number of vars in vector      */
    FUNINT		scnt;		/* in: number of sym tabs following  */
    struct SYMTAB 	*symtab;	/* in: ptr to array of symtab ptrs   */
#endif
    {
    struct  PARHDR 	ph;		/* parameter file header	     */
    struct  LARGE_PARBLK p;		/* parameter block		     */
    ...
    va_list symtab;
#else
    struct  SYMTAB	**st;		/* ptr to array of symtabs	     */
#endif
    struct  SYMTAB	*sym;
    COUNT		i;
    struct  VARIABLE	*v, *vx;
    GENPTR          	r_top();	/* highest byte allocated	     */
...
/* Pack all the variables in 'vector' into as many parblks as necessary.  */
    ...
    for (i=0; i < vcnt ; i++)
    	{
    	vx = vector[i];
    	code = pblk_out(sf, &p, sizeof (p.pool), vector[i]);
    	if (code != SUCCESS)
    	    return (code);
    	}
...
/* Pack all the variables in each 'symtab' into as many parblks as necessary.  */
    ...
    for (i = 0; i < scnt; i++)
    	{
        sym = va_arg(symtab, struct SYMTAB *);
#else
    st = &symtab;
    for (sym = *st++, i = 0; i < scnt; i++, sym = *st++)
    	{
#endif
	for (v=(*sym).link; v!=NULL; v = (*v).v_link)
		...
		continue;
	    code = pblk_out(sf, &p, sizeof(p.pool), v);
	    if (code != SUCCESS)
		return (code);
    	    }
    	}
 ...
 *
 */
    FUNCTION CODE pblk_out (sf, par, poolsize, var)
    ...
    CODE		code;
    GENPTR          	r_top();	/* highest block allocated	 */
...
#define PARBLK_HEADER_SIZE   ((GENPTR)(*par).pool - (GENPTR)par)
	...
	(*par).blksiz = r_top((*par).pool) - (GENPTR)par ;
    	...
    	if (code != SUCCESS)
    	    return (code);
        zero_block (par, PARBLK_HEADER_SIZE + poolsize); 
