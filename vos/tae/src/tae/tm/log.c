...
#include	"parblk.inc"
    ...
    GLOBAL	v43log = 0;		/* source version			*/
...
"PROTOCOL",V_STRING, 0,1,   1,   24,  1, (GENPTR)&netValid,
						      (GENPTR)tcp
	};
 ...
 */
	...
	CODE disable_do(), enable_do(), putmsg_do(),
	     enable_int_do (), disable_int_do (), disable_host_do (),
	     write_do(), enable_recvar(), disable_recvar(),
             enable_facelift_do(), disable_facelift_do();

	CODE enable_force_do(), disable_force_do();
 ...
 */
    ...
    FUNCTION CODE enable_do (cpctx, npctx)
...
de_operr:
    tmmsg(PROCFAIL, "Unable to open script file. %s.",
		"TAE-OPNRD", script.errmsg);
    return(DO_CHECK);

de_tolerr:
    if (code == FAIL)
        tmmsg(PROCFAIL, "Terminal logging already enabled.", "TAE-TOLENB");
    else
...
de_slerr:
    tmmsg(PROCFAIL, "Unable to open session log file. %s.",
		"TAE-OPNWRT", sesslog.sfile.errmsg);
    return(DO_CHECK);
#endif
 ...
 */
	...
	FUNCTION CODE disable_force_do ()
	{
	f_force_lower(FALSE);
	return (DO_CHECK);
	}
 ...
 */
    ...
    FUNCTION CODE putmsg_do(cpctx, npctx)
    ...
    m_pfmt(SVAL(*v1, 0), SVAL(*v2, 0), msgtext); /* format message	    */
 ...
 */
    ...
    FUNCTION CODE write_do (cpctx, npctx)
...

    FUNCTION CODE enable_facelift_do (cpctx, npctx)
...

    FUNCTION CODE disable_facelift_do (cpctx, npctx)
