...

    GLOBAL	v374tm = 0;		/* source version		*/
...
"Re-enter the command line or type TUTOR to recover specified values.";
    ...
    static TEXT	save_file[FSPECSIZ+1];  /* file to save/restore TAE context */
    BOOL		s_equal();	/* strings equal (case insignificant)	*/
    BOOL		s_lseq();	/* 1st string left substring of 2nd?	*/
    COUNT		s_index();	/* get index of char in string		*/
    struct VARIABLE	*allvar();	/* allocate variable & put in symbol tab*/
    GENPTR		allval();	/* alloc space for values (ptrs if string type)*/
    CODE		lookab();	/* find parm in sym tab by unique abbrev*/
    TEXT		*s_save();	/* allocate string into dynamic memory	*/
    BOOL		allprm();	/* TRUE if all parms specd or defaulted	*/
    COUNT		misprm();
    CODE		c_init();	/* init task comm & cntrl pkg		*/
    struct VARIABLE	*search();	/* search for variable in all tables	*/
    struct VARIABLE	*lookex();
    CODE		dotutr();
    CODE		chk_do();

    static BOOL		call_do();
    static VOID		echo(), tr_echo();
 ...
 */
    ...
    IMPORT VOID 	substitute ();  /* cmd line substitution logic  */

    CODE		 code;
    TEXT	net_id[STRINGSIZ+1];	/* TM's network id, for unique filenm */
    struct FSBLOCK tmp_block;


    host_init();			/* host-dependent initialization   */
    d_setsub (substitute);		/* enable MDF/HLP substitution	   */
    d_setctx (&primctx);		/* enable MDF/HLP substitution	   */
 ...
 */
    ...
    FUNCTION  static  VOID local_tm(argc, argv)
	...
	set_string (run_gbl, "INTERACTIVE");
	    ...
	    {
	    tmmsg (SUCCESS, "Unknown terminal type.", "TAE-UNRECTTY");
	    t_bell ();
	    }
	}
#ifdef BUG_FIXES
 ...
 */
    ...
    FUNCTION VOID bchlog_cmd(cmdstr)
 ...
 */
    ...
    FUNCTION CODE bldcmd (inbuf, cmdstr)
 ...
 */
    ...
    FUNCTION static BOOL call_do(ctx, itrcmd)
    ...
    else if (!(*ctx).inbody)		/* if pre-body and no search...	*/
        return (TRUE);			/* always call 			*/
    else if (true_part(ctx))		/* if true part of if...	*/
        return (TRUE);			/* always call			*/
    else
 ...
 */
    ...
    FUNCTION  static  VOID  check_asy_req()
 ...
 */
    ...
    FUNCTION  static  VOID  chk_asy_done()
	...
	if ( (*acb).waiting  &&  !(*acb).active )
		...
		tmmsg (SUCCESS, job_done_msg, "TAE-ASYNCDONE");
	    }
 ...
 */
    ...
    FUNCTION CODE chk_do(ctx, itrcmd)
...
mode_err:
    ...
    tmmsg(PROCFAIL, "'%s' not available in %s mode.",
		"TAE-BADCMD", (*itrcmd).cmd, mode_name);
    return (FAIL);
    }	

/*
 ...
 */
    ...
    if ((*pctx).acbptr != NULL)	/* if there's an ACB, release it	*/
    	free_acb((*pctx).acbptr);

    if ((*pctx).int_procs != NULL)	/* internal proc blocks		*/
    	for (ipbpt = (struct IPBLOCK *)(*pctx).int_procs; ipbpt != NULL; ipbpt = (*ipbpt).flink)
    	    tae_free (ipbpt);
 ...
 */
    ...
    FUNCTION static struct VARIABLE *comp_search (name, pctx)
 ...
 */
    ...
    FUNCTION static VOID echo (ctx, cmdstr)
 ...
 */
    ...
    FUNCTION static VOID tr_echo (ctx, proc)
 ...
 */
    ...
    FUNCTION CODE getcln (prompt, cmdstr)
    ...
    IMPORT struct SFILE  *scr_file;	/* include file pointer		*/
 ...
 */
    ...
    FUNCTION VOID history_save(cmdstr)
 ...
 */
    ...
    zero_block((*pctx).refs, MAXREF * sizeof (GENPTR));	/* init global ref pointers	*/
	...
	if (addstr(&((*pctx).locst), "_STDOUT", 2, 2, standout, V_LOCAL)
 ...
 */
    ...
    FUNCTION  static CODE  init_localjob(run_type)
 ...
 */
    ...
    FUNCTION  static CODE  init_utility()
    {
    IMPORT struct SFILE  stdo_fil;	/* Actual stdout SFILE 		*/
    IMPORT struct SFILE	 *pstdo_fil;	/* Pointer to stdout SFILE 	*/
    IMPORT struct VARIABLE *run_gbl;	/* ptr to runtype global var    */
 ...
 */
    ...
    FUNCTION CODE memtab (resvar, numres, s)
 ...
 */
    ...
    FUNCTION COUNT misprm (symtab, mislst)
 ...
 */
    ...
    if (addstr(&((*pctx).locst), "_STDOUT", 2, 2, standout, V_LOCAL) != SUCCESS)
	goto pl_overr;				/* add stdout name		*/
    if (addstr(&((*pctx).locst), "_ONFAIL", 2, 1, retern, V_LOCAL) != SUCCESS)
	goto pl_overr;
    vptr[0] = (*pctx).pdf.name;
    if (addstr(&((*pctx).locst), "_PROC", 1, 1, vptr, V_LOCAL) != SUCCESS)
	goto pl_overr;
    vptr[0] = (*pctx).subcmd;
    if (addstr(&((*pctx).locst), "_SUBCMD", 1, 1, vptr, V_LOCAL) != SUCCESS)
	goto pl_overr;
    if (addint(&((*pctx).locst), "_TUTOR", 0, V_LOCAL) != SUCCESS)
	goto pl_overr;
    return(SUCCESS);
 ...
 */
    ...
    FUNCTION  static CODE prc_nextcmd(next_cmd, usermode, primctx)
 ...
 */
    ...
    FUNCTION CODE prccmd (cmdstr, procctx)
...

    ...
    active = (*procctx).srchblk == NULL
             &&  true_part(procctx);
    we_care = active;
    code = substitute (cmdstr, CMDLINSIZ, procctx, we_care); /* perform substitution		*/
...
#ifdef TAE_ASYNC
	    code = parmrg (&asy_parmst.link, &(cmdctx.parmst), FULLSET);
...
syntax_err:
    tmmsg(PROCFAIL, sb.errmsg, "TAE-FMTERR");		/* report error			*/
    goto close_return;
 ...
 */
    ...
    FUNCTION CODE repval (v, value, count, by_name)
    ...
    GENPTR		valvec;		/* pointer to value vector	*/
    COUNT		i;
    COUNT		val_count;	/* value count (of ref var)	*/
    struct VARIABLE	*ref;		/* reference			*/
...
ambig_error:
    tmmsg(PROCFAIL, "Value ambiguous for '%s'.", "TAE-AMBIGVAL", (*v).v_name);
    goto err_exit;
 ...
 */
	...
	if (code == SOME_REJECTED)
	    {
	    tmmsg(SUCCESS, errmsg, "TAE-NOTUSED");  /* information only	*/
	    code = SUCCESS;
	    }	
	else if (code != SUCCESS)
	    tmmsg (PROCFAIL, errmsg, "TAE-BADPFILE");
	}
    return (code);
    }

 ...
 */
    ...
    FUNCTION CODE runtype (qualst)

    struct SYMTAB 	*qualst;		/* in: qualifier symtab	*/

    {
    struct VARIABLE		*v, *vn, *lookex();
    TEXT			*value;
 ...
 */
	...
	if (code != SUCCESS)
	    tmmsg(PROCFAIL, errmsg, "TAE-SAVERR");
	}
    return (code);
    }

 ...
 */
    ...
    FUNCTION static VOID save_p_qual (par_ctx, cmd_ctx)
	...
	*********/
	...
	for (v = (*qualtab).link;  v != NULL;  v = (*v).v_link)
	    if ((*v).v_type == V_NAME)
		delvar (qualtab, v);
	}
    return;
 ...
 */
    ...
    FUNCTION CODE updtab (symtab, sb)
...
ud_gverr:
    tmmsg(PROCFAIL, (*sb).errmsg, "TAE-INVPVAL");
    return (FAIL);
...
*/
    tmmsg(PROCFAIL, "Invalid keyword syntax", "TAE-KEYSYNTAX");
    return (FAIL);
...
bad_keyword:
    ...
    tmmsg (PROCFAIL, msg, "TAE-KEYWORD", value[0]);
    return (FAIL);

ud_gkerr:
    tmmsg(PROCFAIL, (*sb).errmsg, "TAE-INVPNAME");
    return (FAIL);
