...
#include 	"tminc.inc"	/* TM defs				*/
    ...
    GLOBAL	vtms112 = 0;	/* source version			*/
    ...
    VOID		tmmsg();
    VOID		oprattn();
    BOOL		s_equal();	/* TRUE if strings are identical*/
 ...
 */
    FUNCTION CODE chg_stdout(procctx)
 ...
 */
    ...
    FUNCTION  CODE  list_missing (symtab)
 ...
 */
    ...
    FUNCTION CODE parcmp (st1, st2, errmsg)
    ...
    for (v1 = (*st1).link; v1 != NULL; v1 = (*v1).v_link)
	{
		...
		if (qual_code != SUCCESS)	/* append differently for quals	*/
		    ...
		    code = SOME_REJECTED;
		    (*v1).v_class = NO_CLASS;	/* force unrecognized class */
		    }
		}
	    }
	if (badpar)
 ...
 */
    ...
    FUNCTION CODE parmrg (st1, st2, mode)
	...
	if ((valid1 = (struct S_VALID *) (*v1).v_valid) != NULL  
	    ...
	    valid2 = allleg (v2, (*valid1).count);  /* alloc/link new struct*/
#ifdef POINTER_VALIDS
	...
	{
	for (v2 = (*st2).link; v2 != NULL; v2 = (*v2).v_link)
	    {
...
over_err:
    tmmsg(PROCFAIL, msg_mmov, key_mmov);
    return(FAIL);
    }
 ...
 *
 */
    FUNCTION CODE re_stdout(cmdctx)
 ...
 */
    ...
    FUNCTION CODE rest_parm(restspec, pctx, errmsg)
	...
	makeabs(&p.symtab, p.pool);	/* make pointers absolute	*/
 	if (chk_parblk(&p) != SUCCESS)	/* check internal integrity	*/
	    goto bad_format;
 ...
 */
    ...
    FUNCTION CODE save_parm (savespec, pctx, access, errmsg)
    ...
    if (code != SUCCESS)
	goto open_err;
    code = save_pfile (&f, 0, NULL, 0, 1, &(*pctx).parmst);
    if (code != SUCCESS)
	goto write_err;
    f_close (&f, F_KEEP);
    return (SUCCESS);
 ...
 */
    ...
    FUNCTION BOOL search_vector (string_vv, count, search_string)
...
qual_err:
    tmmsg(PROCFAIL, (*sb).errmsg, "TAE-BADPQUAL");
    return (FAIL);
 ...
 *	     "CREATE" or "APPEND"
 */
    FUNCTION CODE set_stdout (procctx, cmdctx)
 ...
 */
    ...
    FUNCTION CODE hierarchy(sfile, verbstring, type, lun, fsblock, errstr)
 ...
 */
    ...
    FUNCTION static CODE open_try(sfile, lun, fsblock)
 ...
 */
    ...
    FUNCTION VOID m_put (control, key, a1, a2, a3, a4, a5)

    TEXT 	control[];		/* in: control string		*/
    TEXT	key[];			/* in: message key		*/
    FUNINT	a1, a2, a3, a4, a5;	/* in: integers or string ptrs	*/
 ...
 */
    ...
    FUNCTION VOID m_cput (line, control, key, a1, a2, a3, a4, a5)
 ...
 * pointer to stdo_fil).
 */
    FUNCTION CODE opn_stdout(spec, access)
 ...
 */
    ...
    FUNCTION  CODE  put_outmsg(string)
 ...
 */
    ...
    FUNCTION VOID put_stdout(record)
 ...
 */
    ...
    FUNCTION VOID tmierr (number)

    FUNINT		number;		/* in:  error number		*/

    {
    tmmsg(PROCFAIL, "TAE internal error - %d", "TAE-INTERR", number);
    return;
    }

 ...
 */
    ...
    FUNCTION VOID tmmsg (scode, msg, key, a1, a2, a3, a4, a5)

    FUNINT		scode;		/* in:  status code to set		*/
    TEXT		msg[];		/* in:  message control text		*/
    TEXT		key[];		/* in:  message key			*/
    FUNINT		a1, a2, a3, a4, a5;	/* in:  ints or str ptrs	*/
    ...
    IMPORT CODE	usermode;			/* send menu errs back	*/
    ...
    */
	...
	svv[0] = mputrec;				
	if (chk_vector (_message, V_STRING, svv, 1, FALSE) == SUCCESS)
	    set_value (_message, svv, 1);
	}
    else
 ...
 */
    ...
    FUNCTION VOID tmmsg1 (scode, msg, key, a1, a2, a3, a4, a5)
