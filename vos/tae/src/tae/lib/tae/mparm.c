...
#include	"forstr.inp"
    ...
    GLOBAL v15mparm = 0;			/* version number */
 ...
 */
    ...
    FUNCTION    VOID  p_msgout(msgbuf, msgkey)
...
#ifdef UNIX
    FOR_NAME(getlun_tae) (&lun);	/* get lun # for stdout		*/
#else
    getlun_tae(&lun);		
#endif
...
#ifdef  UNIX
	FOR_NAME(wrtstd) (stdbuf, (long)length); /* pass as a char string */
#else					/* for VAX/VMS and others*/
	wrtstd(stdbuf, &length);	/* pass c string to stdout */
#endif
	}
    return;
    }
