 ...
 */
// Change Log:
// 10-apr-92	Add ctors for Multiple Display support...ga tech,cew
// 06-jul-92	Patch for SGI C++ compiler bug - can't handle the 'new'
//              statement in an inline function...bth
...
// next constructor is not complete -- they're only here to satisfy the
// compiler -- they will be overwritten by tae's version of them

PainterRep::PainterRep(Display* theDisplay) {
    fillgc = XCreateGC(_world->display(), _world->root(), 0, nil);
    dashgc = XCreateGC(_world->display(), _world->root(), 0, nil);
    fillbg = true;
    overwrite = false;
...
/*
 * SGI C++ (v2.1.1) compiler can't handle the new statement(s) in an
 * inline functions, so only use inlines when the target isn't SGI
 */
...
Painter::Painter(Display* theDisplay) {
    display = theDisplay;
    rep = new PainterRep(display);
    Init();
}   /* dual */
