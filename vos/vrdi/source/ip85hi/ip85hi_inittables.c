 ...
 *
 *	Calling Sequence:
 *		STATUS = IP85HI_InitTables(Unit, lun)
 *
 *	Parameter List:
...
#define IP85HI_INITIALIZE
#include "ip85hi.h"

FUNCTION IP85HI_InitTables(Unit, lun)
int *Unit;
short lun;
{
AVAIL_CONFIGS = IMP_1024_BIT | IMP_2048_BIT | VIDEO_1024_BIT | ASPECT_1_1_BIT |
...
MAX_LUT_VALUE       = 255;
EACH_IMP_ZOOMS      = TRUE;
MAY_CONNECT_IMP_LUT = TRUE;	/* sort of... only connect different sections */
MAY_ZOOM_IMPS       = TRUE;
MAX_ZOOM_FACTOR     = 8;

MAY_CONNECT_IMP_OVERLAY = TRUE;	/* sort of... only connect different sections */
MAY_BYPASS_OVERLAY_LUT  = TRUE;	/* not too useful... but possible */
OVERLAY_IMP             = 4;
OVERLAY_LUT_CHAR        = SEPERATE_LUT;
N_OVERLAY_SECTIONS      = 4;
BITS_IN_OVERLAY		= 8;
