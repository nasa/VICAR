 ...
 *	Calling Sequence:
 *
 *		STATUS = xdiiarithmetic( parameters )
 *
 *	Parameter List:
 *
...
FUNCTION FTN_NAME(xdiiarithmetic)( Unit, Operation, Imp1, Imp2, Imp3 )
INTEGER	Unit, Operation, Imp1, Imp2, Imp3;
   {
   return( zdiiarithmetic( *Unit, *Operation, *Imp1, *Imp2, *Imp3 ) );
   }

FUNCTION zdiiarithmetic( unit, operation, imp1, imp2, imp3 )
int	unit, operation, imp1, imp2, imp3;
   ...
   if (!ZCHECK_UNIT_NUMBER) {
      ...
      status = XD_Device_Interface( &unit, ARITHMETIC_OPERATION, operation,
                                    imp1, ZAW(imp1),
                                    imp2, ZAW(imp2),
                                    imp3, ZAW(imp3) );
      ...
      if (status == DEVICE_CANNOT_DO_IT) {
         ...
         for ( i = 0, line1 = ZAW_TOP(imp1),
            ...
            for ( sample = 0; sample < len1; sample++ ) {
               if (operation) {
                  pix3[sample] = MAX(0, (int)pix2[sample]-(int)pix1[sample] );
                  }
               else {
                  pix3[sample] = MIN(255, (int)pix2[sample]+(int)pix1[sample] );
                  }
               }
            status = XD_Device_Interface( &unit, WRITE_LINE, imp3,
                                          ZAW_LEFT(imp3), line3, len1, 
                                          pix3, ALL_BITS );
            if ( status != SUCCESS ) break;
         }
