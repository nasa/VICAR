...
<xsl:if test="count(subitem)>0">
  ...
  <xsl:when test="@key='IMAGE_RADIANCE_OFFSET'"> 
   <xsl:element name="item"> 
	...
	<xsl:when test="@key='INSTRUMENT_TEMPERATURE_STATUS'"> 
  	...
  	<xsl:when test="@key='INTERPOLATION_METHOD'"> 
   		<xsl:element name="item"> 
    	<xsl:attribute name="key">MSL:INTERPOLATION_METHOD</xsl:attribute> 
    	<xsl:attribute name="quoted"><xsl:value-of select="@quoted"/></xsl:attribute> 
    	<xsl:if test="normalize-space(@units)">  
       	<xsl:attribute name="units"><xsl:value-of select="@units"/></xsl:attribute> 
    	</xsl:if> 
  	...
  	<xsl:when test="@key='INTERPOLATION_VALUE'"> 
   		<xsl:element name="item"> 
    	<xsl:attribute name="key">MSL:INTERPOLATION_VALUE</xsl:attribute> 
    	<xsl:attribute name="quoted"><xsl:value-of select="@quoted"/></xsl:attribute> 
    	<xsl:if test="normalize-space(@units)">  
       	<xsl:attribute name="units"><xsl:value-of select="@units"/></xsl:attribute> 
    	</xsl:if> 
    ...
    <xsl:when test="@key='MODEL_COMPONENT_UNIT'"> 
    <!--  
       <xsl:element name="item"> 
       <xsl:attribute name="key">MODEL_COMPONENT_UNIT</xsl:attribute> 
       <xsl:attribute name="quoted"><xsl:value-of select="@quoted"/></xsl:attribute> 
       <xsl:if test="normalize-space(@units)">  
        <xsl:attribute name="units"><xsl:value-of select="@units"/></xsl:attribute> 
       </xsl:if>
       ...
       -->
       		<item key="MODEL_COMPONENT_UNIT" quoted="true">
			<subitem key="MODEL_COMPONENT_UNIT" quoted="true">meter</subitem>
			<subitem key="MODEL_COMPONENT_UNIT" quoted="true">N/A</subitem>
			<subitem key="MODEL_COMPONENT_UNIT" quoted="true">pixel</subitem>
			<subitem key="MODEL_COMPONENT_UNIT" quoted="true">pixel</subitem>
		</item>
    </xsl:when>  
  ...
  <xsl:when test="@key='MAP_SCALE'"> 
   <xsl:element name="item"> 
    <xsl:attribute name="key">MAP_SCALE</xsl:attribute> 
    <xsl:attribute name="quoted"><xsl:value-of select="@quoted"/></xsl:attribute> 
    <xsl:if test="normalize-space(@units)">  
    <!--  
       <xsl:attribute name="units">pixel/degree</xsl:attribute> 
  ...
  <xsl:when test="@key='RADIANCE_OFFSET'"> 
   ...
   <!-- 
    <xsl:attribute name="key">RADIANCE_OFFSET</xsl:attribute> 
    -->
    <xsl:attribute name="key">MSL:RADIANCE_SCALING_FACTOR</xsl:attribute> 
    <xsl:attribute name="quoted"><xsl:value-of select="@quoted"/></xsl:attribute> 
    <xsl:if test="normalize-space(@units)"> 
       <xsl:attribute name="units">W.m**-2.sr**-1.nm**-1</xsl:attribute>       
    </xsl:if> 
  ...
  <xsl:when test="@key='REFERENCE_AZIMUTH'"> 
   <xsl:element name="item"> 
    <xsl:attribute name="key">REFERENCE_AZIMUTH</xsl:attribute> 
    <xsl:attribute name="quoted"><xsl:value-of select="@quoted"/></xsl:attribute> 
    <!--  REMOVE UNITS -->
    <xsl:apply-templates/>  
   </xsl:element> 
...
<!-- 6-4-2012 added INST_CMPRS_SEGMENT_STATUS MSL:INST_CMPRS_SEGMENT_STATUS -->
  ...
  <xsl:when test="@key='DATA_SET_MAP_PROJECTION'"> 
       <xsl:element name="item"> 
       <xsl:attribute name="key">^DATA_SET_MAP_PROJECTION</xsl:attribute> 
       <xsl:attribute name="quoted"><xsl:value-of select="@quoted"/></xsl:attribute> 
       <xsl:value-of select="."/>  	 
       </xsl:element> 
  </xsl:when> 
...
<xsl:template name="fixUnits">
  ...
  <xsl:when test="$unitString='<DEGREES>'">   
   -->
    ...
    <xsl:when test="contains($unitString,'M/PIXELS')"> 
       <xsl:call-template name="globalReplace">
  		<xsl:with-param name="outputString" select="$unitString"/>
  		<xsl:with-param name="target" select="'M/PIXELS'"/>
  		<xsl:with-param name="replacement" select="'m/pixels'"/>
  	   </xsl:call-template>    
    </xsl:when>
    
    <xsl:when test="contains($unitString,'M/PIXEL')"> 
       <xsl:call-template name="globalReplace">
  		<xsl:with-param name="outputString" select="$unitString"/>
  		<xsl:with-param name="target" select="'M/PIXEL'"/>
  		<xsl:with-param name="replacement" select="'m/pixel'"/>
  	   </xsl:call-template>    
    </xsl:when>
