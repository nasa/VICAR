	...
	<xsl:template match="PROPERTY[@name='CHEM_REQUEST']">		
        ...
        <OBJECT>
			...
			<xsl:element name="item">
				...
				<xsl:attribute name="quoted">false</xsl:attribute><xsl:value-of select="@name"/></xsl:element>
			<xsl:apply-templates/>
		</OBJECT>
	</xsl:template>
...
<xsl:element name="subitem">
  <xsl:choose>
	<xsl:when test="contains(../following-sibling::item/subitem/@name, '__UNIT')">
		<xsl:attribute name="unit"><xsl:value-of select="normalize-space(../following-sibling::item/subitem[@name])"/></xsl:attribute>	
	</xsl:when>
  </xsl:choose>
