...
<xsl:stylesheet version="2.0"
				...
				<xsl:choose>
					...
					<xsl:when test="not(contains(@name, '__UNIT'))">
						<xsl:element name="item">
							...
							<!-- try to locate an item which is. must be the same parent ?? <xsl:template 
								match="item[@name='HISTOGRAM_REQUEST_PARMS']"> <comment>/* HISTOGRAM REQUEST 
								*/</comment> <GROUP> <xsl:attribute name="name"><xsl:value-of select="@name"/></xsl:attribute> 
								<xsl:apply-templates/> </GROUP> </xsl:template> <xsl:if test="contains(following-sibling::item/@name,'__UNIT')"> 
								normalize-space <xsl:when test="contains(../parent::item/@name, '__UNIT')"> 
								<xsl:if test="normalize-space(parent::item/@name,'__UNIT')"> <xsl:attribute 
								name="unit"><xsl:value-of select="concat(parent::item[@name],'__UNIT')"/></xsl:attribute> 
								-->
							...
							<xsl:if test="//../item[@name=$name_units]">
								<!--  
								<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
								<xsl:attribute name="found_unit">true</xsl:attribute> 
								-->

								<xsl:attribute name="unit"><xsl:value-of
									select="//../item[@name=$name_units]" /></xsl:attribute>
								<!-- <xsl:value-of select="."/> -->
							</xsl:if>
							...
							<xsl:choose>
								...
								<!-- <xsl:when test="@name='AZIMUTH_FOV'"> <xsl:attribute name="name_unit" 
									><xsl:value-of select="$name_unit"/></xsl:attribute> <xsl:attribute name="name_units"><xsl:value-of 
									select="$name_units"/></xsl:attribute> <xsl:attribute name="name_unit_parent"><xsl:value-of 
									select="$name_unit_parent"/></xsl:attribute> <xsl:attribute name="name_unit_parent_name"><xsl:value-of 
									select="$name_unit_parent_name"/></xsl:attribute> <xsl:attribute name="unit"><xsl:value-of 
									select="//../item[@name=$name_units]"/></xsl:attribute> <xsl:value-of select="."/> 
									</xsl:when> -->
	...
	<xsl:template match="subitem">
		...
		<xsl:element name="subitem">
			<!-- <xsl:choose> <xsl:when test="contains(../following-sibling::item/subitem/@name, 
				'__UNIT')"> <xsl:attribute name="unit"><xsl:value-of select="normalize-space(../following-sibling::item/subitem[@name])"/></xsl:attribute> 
				</xsl:when> </xsl:choose> -->
			...
			<xsl:if test="//../subitem[@name=$name_units]">				
			    <xsl:attribute name="unit"><xsl:value-of select="//../subitem[@name=$name_units]" /></xsl:attribute>
			</xsl:if>
			
			<!--  			
			<xsl:choose>
				<xsl:when test="contains(../subitem/@name, '__UNIT')">
					<xsl:attribute name="found_subitem_unit">true</xsl:attribute>
					<xsl:attribute name="unit"><xsl:value-of
						select="concat(../subitem[@name], '__UNIT')" /></xsl:attribute>
				</xsl:when>
			</xsl:choose>
			-->
