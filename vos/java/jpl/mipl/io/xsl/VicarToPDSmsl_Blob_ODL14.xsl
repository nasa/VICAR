...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="VICAR_LABEL">
		<PDS_LABEL>
			...
			<!-- IMAGE_MAP_PROJECTION  new 2-2016 -->
			<xsl:apply-templates select="PROPERTY[@name='IMAGE_MAP_PROJECTION']" />
	...
	<xsl:template match="PROPERTY[@name='IMAGE_MAP_PROJECTION']">
		<comment>/* IMAGE MAP PROJECTION DATA ELEMENTS */</comment>
		<OBJECT>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</OBJECT>
					...
					<xsl:when test="not(contains(@name, '__UNIT'))">
						<xsl:element name="item">
							...
							<xsl:choose>
								...
								<!--  DATA_SET_MAP_PROJECTION_CAT__PTR ^DATA_SET_MAP_PROJECTION_CATALOG-->
															
								<xsl:when test="@name='DATA_SET_MAP_PROJECTION__PTR'">
								<xsl:attribute name="name">^DATA_SET_MAP_PROJECTION</xsl:attribute>
