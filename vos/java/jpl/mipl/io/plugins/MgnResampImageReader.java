/**
 * Image Reader for Magellan Resamp format.  This is a format used by
 * Scott Hensley's software to analyze Magellan stereo data.  It is basically
 * a subset of an F-BIDR in image format (as floats) with a small header
 * in front.
 * <p>
 * Code based on PdsImageReader.java
 * <p>
 * This class is an <code>ImageReader</code> for reading image files in the
 * Magellan Resamp format.  We use the VICAR I/O mechansim because once you
 * get past the header, it is a simple raster of floats.
 *
 * @version 0.1
 *
 * @author Bob Deen, JPL
...
public class MgnResampImageReader extends ImageReader {
 ...
 */
    public MgnResampImageReader (ImageReaderSpi mySpi) {
        super(mySpi);
        if (debug)   {
	    System.out.println("**********************");  
            System.out.println("MgnResampImageReader constructor");
    	}
    ...
    public void setInput(Object input, boolean isStreamable) {
	if (debug)     
	    System.out.println("MgnResampImageReader public void setInput(Object input, boolean isStreamable)");
    ...
    public void setInputInternal(Object input) { 
        if (debug) {
	    System.out.println("MgnResampImageReader.setInputInternal "+input);
        }
 ...
 */
    public RenderedImage readAsRenderedImage(int imageIndex,
        ...
        if (debug)
	    System.out.println("MgnResampImageReader.readAsRenderedImage()");
        if (imageIndex != 0) {
	...
	if (debug) 
            System.out.println("MgnResampImageReader.readAsRenderedImage() after readHeader() ");
	...
	if (_haveReadHeader)
        ...
        if (debug) {   
	    System.out.println("MgnResampImageReader.readHeader");
	...
	// The MGN Resamp format only supports float data, and single band
	// (BSQ), so this is much simpler than for the sibling formats.

        _imageType = null;
        SampleModel sampleModel = _mif.createSampleModel(_sys.getNS(),
                                                         _sys.getNL());
        ColorModel colorModel = PlanarImage.createColorModel(sampleModel);
        if (debug) {
            System.out.println("after mif.createSampleModel()");
            System.out.println("sampleModel "+sampleModel);
            System.out.println("colorModel "+colorModel);
        }
        try {
            _imageType = new ImageTypeSpecifier(colorModel, sampleModel);
        }
        catch (IllegalArgumentException iae) {
            if (debug) {
                System.out.println("MgnResampImageReader.readHeader() ImageTypeSpecifier");
	...
	if (document != null) {
            if (debug) {
            	System.out.println("MgnResampImageReader new MgnResampMetadata with document");
            	System.out.println("+++++++++++++++++++++++++++++++++++++++++++++++");
            }
            _mgnResampMetadata = new MgnResampMetadata(document,_mif.getHeader());
	} else {
	    if (debug) {
            	System.out.println("no document avaiable from _mif");
            	System.out.println("MgnResampImageReader NO MgnResampMetadata **********");
       	    }
            return;
	}
    ...
    */
	...
	if (debug) System.out.println("MgnResampImageReader.readMetadata");

        readHeader();
    }

    public String getFormatName()  throws IOException {
	if (debug) System.out.println("MgnResampImageReader.getFormatName");
        return "mgn-resamp";
    }

    public int getNumImages() throws IOException {
	if (debug) System.out.println("MgnResampImageReader.getNumImages");
        return 1;			// we only support 1
    }
    
    public int getWidth(int imageIndex) throws IOException {
	if (debug) System.out.println("MgnResampImageReader.getWidth("+imageIndex+")");
        if (imageIndex != 0) {
            throw new IndexOutOfBoundsException();
        }
        readHeader();
	...
	if (debug) System.out.println("MgnResampImageReader.getHeight("+imageIndex+")");
        if (imageIndex != 0) {
            throw new IndexOutOfBoundsException();
        }
        readHeader();
    ...
    public ImageTypeSpecifier getRawImageType(int imageIndex)
						throws IOException {
	if (debug) System.out.println("MgnResampImageReader.getRawImageType");
        if (imageIndex != 0) {
            throw new IndexOutOfBoundsException();
        }
        readHeader();
	...
	if (debug) System.out.println("MgnResampImageReader.getImageTypes");
        if (imageIndex != 0) {
            throw new IndexOutOfBoundsException();
        }
        readHeader();
	...
	if (debug)
	    System.out.println("MgnResampImageReader.getDefaultReadParam");
	...
	if (debug) System.out.println("MgnResampImageReader.getStreamMetedata");
        return null;
    }

    public IIOMetadata getImageMetadata(int imageIndex) throws IOException {
	if (debug) System.out.println("MgnResampImageReader.getImageMetadata("+imageIndex+")");
        if (imageIndex != 0) {
            throw new IndexOutOfBoundsException("imageIndex != 0!");
        }
        readMetadata(); // make sure _mgnFbidrMetadata has valid data in it
        return _mgnResampMetadata;
    }

    public void printParam(ImageReadParam param) {
        System.out.println("------------------------------------------");
        System.out.println("MgnResampImageReader ImageReadParam:");
...
System.err.println("NTT param = "+param.getClass());
        if (debug) {
	    System.out.println("*** read read read read read *******");
	    System.out.println("************************************");
            System.out.println("MgnResampImageReader.read("+imageIndex+")");
       	}
        if (imageIndex != 0) {
            throw new IndexOutOfBoundsException ();
        }
        ...
        if (debug) System.out.println("MgnResampImageReader.read() after readHeader() ");
        /**
	...
	if (debug) {
	    ...
	    System.out.println("MgnResampImageReader.read() _imageType "+_imageType);
        }
	    ...
	    if (debug) {
        	System.out.println("MgnResampImageReader.read() with and height == 0. create an image 1x1");
	    }
        }
	...
	if (parameterizedRead == false && _imageType != null) {
		...
		System.out.println("_imageType.createBufferedImage ");
	        ...
	        if (colorModel == null) {
         	    System.out.println("ERROR  MgnResampImageReader.read() colorModel is null. Can't create BufferedImage");
	        }
                else {
        ...
        if (debug) {
            System.out.println("MgnResampImageReader.read() >>> _mif.readTile >>>>>>>");
        ...
        if (_fakeImageNoRead == false ) {
        	...
        	e.printStackTrace();
        	throw new RuntimeException("IOException occured while reading MGN RESAMP image file");
            } 
        }

        if (debug) {
            System.out.println("MgnResampImageReader.read() completed");
            System.out.println("sampleModel="+sampleModel);
            System.out.println("colorModel="+colorModel);
        }
        return theImage;
	...
	if (debug) System.out.println("MgnResampImageReader.getNumThumbnails");
        return 0;
    }
    ...
    public BufferedImage readThumbnail(int imageIndex,
                                       int thumbnailIndex)
					throws IOException {
	if (debug) System.out.println("MgnResampImageReader.readThumbnail");
